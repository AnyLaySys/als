...
static jboolean requestConnection(__unused JNIEnv
...
struct sockaddr_in server = {.sin_family = AF_INET, .sin_port = htons(
...
check(getsockopt(sock, SOL_SOCKET, SO_ERROR, &so_error, &len) < 0, "getsockopt failed: %s",
      strerror(errno));
if (so_error == ECONNREFUSED) goto
end;
...
static void connect_(__unused JNIEnv
...
static void nativeInit(JNIEnv *env, jobject thiz) {
    ...
    if (!Charset.self) {
        ...
        Charset.forName = FindMethodOrDie(env, Charset.self, "forName",
                                          "(Ljava/lang/String;)Ljava/nio/charset/Charset;",
                                          JNI_TRUE);
        Charset.decode = FindMethodOrDie(env, Charset.self, "decode",
                                         "(Ljava/nio/ByteBuffer;)Ljava/nio/CharBuffer;", JNI_FALSE);

        CharBuffer.self = FindClassOrDie(env, "java/nio/CharBuffer");
        CharBuffer.toString = FindMethodOrDie(env, CharBuffer.self, "toString",
                                              "()Ljava/lang/String;", JNI_FALSE);

        MainActivity.self = FindClassOrDie(env, "com/termux/x11/MainActivity");
        MainActivity.getInstance = FindMethodOrDie(env, MainActivity.self, "getInstance",
                                                   "()Lcom/termux/x11/MainActivity;", JNI_TRUE);
        MainActivity.clientConnectedStateChanged = FindMethodOrDie(env, MainActivity.self,
                                                                   "clientConnectedStateChanged",
                                                                   "()V", JNI_FALSE);
        MainActivity.resetIme = FindMethodOrDie(env, (*env)->GetObjectClass(env, thiz), "resetIme",
                                                "()V", JNI_FALSE);
    }
...
static int xcallback(int fd, int events, __unused void *data) {
    ...
    if (events & (ALOOPER_EVENT_ERROR | ALOOPER_EVENT_HANGUP)) {
        jobject instance = (*env)->CallStaticObjectMethod(env, MainActivity.self,
        ...
        if (read(conn_fd, &e, sizeof(e)) == sizeof(e)) {
            switch (e.type) {
                case EVENT_CLIPBOARD_SEND: {
                    ...
                    jobject charset = (*env)->CallStaticObjectMethod(env, Charset.self,
                                                                     Charset.forName,
                ...
                case EVENT_CLIPBOARD_REQUEST: {
                    (*env)->CallVoidMethod(env, thiz, (*env)->GetMethodID(env,
                                                                          (*env)->GetObjectClass(
                                                                                  env, thiz),
                                                                          "requestClipboard",
                                                                          "()V"));
                ...
                case EVENT_SHARED_SERVER_STATE: {
                    ...
                    state = mmap(NULL, sizeof(*state), PROT_READ | PROT_WRITE, MAP_SHARED, stateFd,
                                 0);
...
ALooper_addFd(ALooper_forThread(), fd,

0, ALOOPER_EVENT_INPUT | ALOOPER_EVENT_ERROR | ALOOPER_EVENT_HANGUP, xcallback, NULL);
}

static jboolean connected(__unused JNIEnv
...
static void setClipboardSyncEnabled(__unused JNIEnv

* env,
__unused jobject
cls,
jboolean enable, __unused
jboolean ignored
) {
if (conn_fd != -1) {
...
static void sendClipboardAnnounce(__unused JNIEnv
...
static void sendWindowChange(__unused JNIEnv
...
jint width, jint
height,
jint framerate, jstring
jname) {
if (conn_fd != -1) {
const char *name = (!jname || width <= 0 || height <= 0) ? NULL : (*env)->GetStringUTFChars(env,
                                                                                            jname,
                                                                                            JNI_FALSE);
lorieEvent e = {.screenSize = {.t = EVENT_SCREEN_SIZE, .width = width, .height = height, .framerate = framerate, .name_size = (name
                                                                                                                               ? strlen(
                name) : 0)}};
write(conn_fd,
&e, sizeof(e));
...
static void sendMouseEvent(__unused JNIEnv
...
if (which_button > 0)
(*env)->
CallVoidMethod(env, globalThiz, MainActivity
.resetIme);
...
static void sendTouchEvent(__unused JNIEnv
...
static void sendStylusEvent(__unused JNIEnv
...
static void requestStylusEnabled(__unused JNIEnv
...
static jboolean sendKeyEvent(__unused JNIEnv
...
static void startLogcatStub(__unused JNIEnv
...
JNIEnv *env;
static JNINativeMethod methods[] = {{"nativeInit",              "()V",                       (void *) &nativeInit},
