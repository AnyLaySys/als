...
import sui.k.als.qemu.kvm.qemuDisplayDeviceArgument
import sui.k.als.qemu.kvm.qemuMemoryArgument
...
fun QemuKvmScreen(
    ...
    val disk = rememberLauncherForActivityResult(ActivityResultContracts.OpenDocument()) {
        it?.let { config = config.copy(diskPath = context.qemuKvmPath(it)) }
    }
    fun save() = QemuKvmConfigStore.save(context, config)
    Box(
        Modifier
            .fillMaxSize()
        ...
        Column(
            ...
            Text(
                stringResource(R.string.qemu_kvm),
                color = Color.White,
                fontSize = 15.sp,
            ...
            Column {
                QemuKvmText(stringResource(R.string.cfg_name), config.name, true) {
                    config = config.copy(name = it)
                }
                QemuKvmNumber(stringResource(R.string.cpu_cores), config.cpuCores) {
            ...
            Column {
                QemuKvmPath(stringResource(R.string.disk_path), config.diskPath, true, true) {
                    disk.launch(arrayOf("*/*"))
                }
            }
            Column {
                QemuKvmDisplayDevice(config, first = true) {
                    config = config.copy(displayDevice = it)
                }
                QemuKvmNumber(stringResource(R.string.xres), config.width) {
            ...
            Column {
                QemuKvmToggle(stringResource(R.string.network), config.network, true, true) {
                    config = config.copy(network = it)
                }
            }
            Column {
                QemuKvmToggle(stringResource(R.string.audio), config.audio, true) {
...
@Composable
private fun QemuKvmDisplayDevice(
    config: QemuKvmConfig,
    first: Boolean = false,
    last: Boolean = false,
    onChange: (String) -> Unit
) {
    ...
    Field(
        ...
        options.first { it.first == config.displayDevice.toQemuKvmDisplayDevice() }.second,
...
private fun Context.qemuKvmPath(uri: Uri): String {
