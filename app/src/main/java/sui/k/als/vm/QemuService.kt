...
class QemuService : Service() {
    ...
    private fun run(value: Request) {
        ...
        try {
            ...
            if (!stopped()) {
                ...
                ALSLog.info("VM", "Preflight started")
                prepared.preflight()
                ALSLog.info("VM", "Preflight passed")
            }
            if (!stopped()) {
    ...
    private fun finishExited(status: Int) {
        ...
        if (status == 0) {
            ALSLog.info("VM", "QEMU returned 0")
        } else {
            ALSLog.info("VM", "QEMU returned $status")
    ...
    private fun finishFailure(error: Throwable) {
        ...
        ALSLog.error("VM", "QEMU launch failed", error)
